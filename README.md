Xpenology AMD hardvare virtualization
  ...
  Making kvm-amd.ko and launch libvirtd, HP MicroServer GEN 10 ( AMD Opteron X3216 APU with svm ( AMD-V ))

TL/DR: Hi guys, i'm try to use vrtualization with hardware acceleration on Hp MicroServer Gen 10 and succesfully launch Ubuntu 18.04 on it. What i cannot do:
 * launch virtual machines via internal VMM manager (error "" )
 * create network for virtual machine (ebtables needed?)
 * 
 
 But i still cannot launch internal Virtual Machine Manager 


1. Install and update debian chroot (you can do it in your own system or on Synology NAS)
...
Chroot to installed package (you need to login by ssh to your Synology, installing chroot takes long time. Check via "ps ajfx" in dom0 that all postinst actions are over):
...
2. Prepare enviroment ( this instruction i'm take from this article https://blog.4sag.ru/kompilyatsiya-modulej-dsm-6-2/ , but kernel version is different )

apt-get install mc make gcc build-essential kernel-wedge libncurses5 libncurses5-dev libelf-dev binutils-dev kexec-tools makedumpfile fakeroot lzma bc libssl-dev openssl vim netcat-openbsd 

mkdir /dsmsrc
cd /dsmsrc

wget --no-check-certificate https://sourceforge.net/projects/dsgpl/files/Synology%20NAS%20GPL%20Source/24922branch/apollolake-source/linux-4.4.x.txz

wget --no-check-certificate https://sourceforge.net/projects/dsgpl/files/DSM%206.2%20Tool%20Chains/Intel%20x86%20Linux%204.4.59%20%28Apollolake%29/apollolake-gcc493_glibc220_linaro_x86_64-GPL.txz

tar -xvf apollolake-gcc493_glibc220_linaro_x86_64-GPL.txz 
tar -xvf linux-4.4.x.txz 
cd linux-4.4.x
...
dsm6make menuconfig ( 1. Go to Virtualization ->  KVM for AMD processors support and put M symbol here; 2. Go to one level up, to Device Drivers -> Network device support -> Universal TUN/TAP device driver support and put M symbol here if it does not exist; 3. Select Save in menu and save this config as .config)
...
3.1 Check that modules was loaded property by "dmesg" command (you can launch this command without grep to see all messages, check the time - when event happend )
...
4. Generate, and copy ssh.pub key from your machine to Xpenology NAS
...
5. Use openbsd syntax netcat (libvirtd required), and some packages and libs, symlink it from chroot
...
7. Click on link and create new virtual machine

Specify additional options
Remove network an usb devices, use vnc graphic card.
...
Use openswitch for network, for nic use config (XML section) like ( see http://docs.openvswitch.org/en/latest/howto/libvirt/ ):
...
Create own bridge device:

oleg@DiskStation:~$ sudo brctl addbr mybridge1
oleg@DiskStation:~$ sudo brctl addif mybridge1 ovs_eth1

In Network selection use "Specify shared device name" and put "mybridge1" here.

8. Go to console to NAS, check qemu virtual machine:

oleg@DiskStation:/lib$ ps ajfx | grep qemu
23514 26196 26195 23514 pts/16   26195 S+    1026   0:00  |           \_ grep --color=auto qemu
    1 24892 24891 24891 ?           -1 Sl       0   0:54 /usr/local/bin/qemu-system-x86_64 -name ubuntu18.04 -S -machine pc-q35-2.12,accel=kvm,usb=off,vmport=off -cpu Opteron_G5 -m 1024 -realtime mlock=off -smp 1,sockets=1,cores=1,threads=1 -uuid 1849696a-2f7b-48b8-affa-df947c048dd4 -no-user-config -nodefaults -chardev socket,id=charmonitor,path=/var/lib/libvirt/qemu/ubuntu18.04.monitor,server,nowait -mon chardev=charmonitor,id=monitor,mode=control -rtc base=utc,driftfix=slew -global kvm-pit.lost_tick_policy=discard -no-hpet -no-shutdown -global PIIX4_PM.disable_s3=1 -global PIIX4_PM.disable_s4=1 -boot strict=on -device i82801b11-bridge,id=pci.1,bus=pcie.0,addr=0x1e -device pci-bridge,chassis_nr=2,id=pci.2,bus=pci.1,addr=0x1 -device ich9-usb-ehci1,id=usb,bus=pci.2,addr=0x2.0x7 -device ich9-usb-uhci1,masterbus=usb.0,firstport=0,bus=pci.2,multifunction=on,addr=0x2 -device ich9-usb-uhci2,masterbus=usb.0,firstport=2,bus=pci.2,addr=0x2.0x1 -device ich9-usb-uhci3,masterbus=usb.0,firstport=4,bus=pci.2,addr=0x2.0x2 -device virtio-serial-pci,id=virtio-serial0,bus=pci.2,addr=0x3 -drive file=/volume1/files/ubuntu-mate-18.04.1-desktop-amd64.iso,format=raw,if=none,media=cdrom,id=drive-sata0-0-0,readonly=on -device ide-cd,bus=ide.0,drive=drive-sata0-0-0,id=sata0-0-0,bootindex=1 -chardev pty,id=charserial0 -device isa-serial,chardev=charserial0,id=serial0 -chardev socket,id=charchannel0,path=/var/lib/libvirt/qemu/channel/target/ubuntu18.04.org.qemu.guest_agent.0,server,nowait -device virtserialport,bus=virtio-serial0.0,nr=1,chardev=charchannel0,id=channel0,name=org.qemu.guest_agent.0 -device usb-tablet,id=input0 -vnc 127.0.0.1:0 -device VGA,id=video0,vgamem_mb=16,bus=pcie.0,addr=0x1 -device ich9-intel-hda,id=sound0,bus=pci.2,addr=0x1 -device hda-duplex,id=sound0-codec0,bus=sound0.0,cad=0 -device virtio-balloon-pci,id=balloon0,bus=pci.2,addr=0x4 -msg timestamp=on
